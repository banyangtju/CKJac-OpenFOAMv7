...
{
/*	const scalarField& T = this->thermo().T().oldTime(); // ofv9 DLBFoam
	const scalarField& p = this->thermo().p().oldTime(); // ofv9 DLBFoam
	tmp<volScalarField> trho(this->thermo().rho0()); // ofv9 DLBFoam*/

	const scalarField& T = this->thermo().T(); // ofv7 DLBFoam
    const scalarField& p = this->thermo().p(); // ofv7 DLBFoam
	tmp<volScalarField> trho(this->thermo().rho()); // ofv7 DLBFoam
    ...
    forAll(T, celli)
    {
        ...
        if(T[celli] > this->Treact())
        {
            ...
            {
//				concentration[i] = rho[celli] * this->Y_[i].oldTime()[celli] / this->specieThermos_[i].W(); // ofv9 DLBFoam
//				massFraction[i] = this->Y_[i].oldTime()[celli]; // ofv9 DLBFoam
				concentration[i] = rho[celli] * this->Y_[i][celli] / this->specieThermo_[i].W(); // ofv7 DLBFoam
                massFraction[i] = this->Y_[i][celli]; // ofv7 DLBFoam
			}
...
)
{
    ...
    {
 //		this->RR_[j][i] = solution.c_increment[j] * this->specieThermos_[j].W(); // ofv9 DLBFoam
		this->RR_[j][i] = solution.c_increment[j] * this->specieThermo_[j].W(); // ofv7 DLBFoam
    }
